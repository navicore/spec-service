...
use spec_server::domain::{
    aggregates::Spec,
    commands::{CreateSpec, PublishSpec, UpdateSpec, DeprecateSpec},
    events::{EventMetadata, SpecEvent, SpecState},
};
use spec_server::infrastructure::{
    event_store::SqliteEventStore,
    projections::ProjectionStore,
    event_processor::EventProcessorManager,
};
use std::sync::Arc;
use uuid::Uuid;
...
async fn main() -> Result<()> {
    ...
    let specs = vec![
        ...
        ("validation-rules", "Input validation patterns", "bob@example.com"),
        ("approval-workflow", "Approval chain configuration", "carol@example.com"),
    ];
    ...
    let draft_specs = projection_store.list_by_state(Some(SpecState::Draft), 10, 0).await?;
    for spec in &draft_specs {
        println!("- {} (v{}) - {}", spec.name, spec.latest_version, spec.description.as_deref().unwrap_or(""));
    }
    ...
    let published_specs = projection_store.list_by_state(Some(SpecState::Published), 10, 0).await?;
    ...
    event_store.append_events(*spec_id, update_events, EventMetadata::default()).await?;
    ...
    if let Some(current) = projection_store.get_by_id(*spec_id).await? {
        println!("\nCurrent version of '{}': v{}", current.name, current.version);
    }
    
    // Query specific version
    if let Some((content, desc)) = projection_store.get_version(*spec_id, 1).await? {
        println!("\nVersion 1 content preview: {}", &content[..50.min(content.len())]);
    }
    ...
    event_store.append_events(*spec_id, deprecate_events, EventMetadata::default()).await?;
    ...
    for spec in all_non_deleted {
        println!("- {} (v{}) - State: {:?}", spec.name, spec.latest_version, spec.state);
    }
