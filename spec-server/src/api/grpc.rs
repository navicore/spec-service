...
use spec_proto::{
    ...
    CreateSpecRequest, CreateSpecResponse, DeprecateSpecRequest, DeprecateSpecResponse,
    EventType, GetSpecHistoryRequest, GetSpecHistoryResponse, GetSpecRequest, GetSpecResponse,
...
impl SpecService for SpecServiceImpl {
    ...
    async fn update_spec(
        ...
        let req = request.into_inner();
        let spec_id = Uuid::parse_str(&req.id)
            .map_err(|_| Status::invalid_argument("Invalid spec ID"))?;
        ...
        Ok(Response::new(UpdateSpecResponse { version: new_version }))
    }
    ...
    async fn get_spec(
        ...
        let spec = if let Some(version) = req.version {
            ...
            let (content, description) = self.projection_store
            ...
            let current = self.projection_store
                .get_by_id(spec_id)
                .await
                .map_err(|e| Status::internal(e.to_string()))?
            ...
            // Get current version
            let spec = self.projection_store
                .get_by_id(spec_id)
                .await
                .map_err(|e| Status::internal(e.to_string()))?
                .ok_or_else(|| Status::not_found("Spec not found"))?;
    ...
    async fn list_specs(
        ...
        let state_filter = req.state.and_then(|s| {
            ProtoSpecState::try_from(s).ok().and_then(proto_state_to_domain)
        });
        
        let page_size = req.page_size as i64;
        let offset = 0; // TODO: Implement page token parsing
        
        let specs = self.projection_store
    ...
    async fn get_spec_history(
        ...
        let event_envelopes = self.event_store
            .get_events(spec_id, None)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
            ...
            .map(|envelope| {
                let (event_type, payload) = match &envelope.event {
                    ...
                    SpecEvent::StateChanged(e) => (
                        ...
                        spec_proto::spec_event::Payload::StateChange(spec_proto::StateChangePayload {
                ...
                ProtoSpecEvent {
                    ...
                    occurred_at: Some(chrono_to_proto_timestamp(get_event_timestamp(&envelope.event))),
